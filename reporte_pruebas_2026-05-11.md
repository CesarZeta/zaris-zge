...
**Usuario de prueba:** Cesar Zeta · Administrador (`ciudadanovl` / `[redactado]`)  
...
**Causa del bloqueo:** Los usuarios `lmartinez@municipio.gob.ar` y `mgonzalez@municipio.gob.ar` devuelven **"Credenciales no coinciden"** en producción con contraseña `[redactado]`.
...
> **Acción requerida:** Verificar que el hash de contraseña `[redactado]` esté correcto en la DB de prod para `id_usuario=7` (lmartinez) e `id_usuario=8` (mgonzalez). O resetear contraseñas manualmente.
...
|---|---|---|---|---|
| 2026-05-11 | B.1, B.4 | `lmartinez` y `mgonzalez` dan 401 en prod con pass `[redactado]`. Usuarios existen en DB pero contraseña no coincide. | Prod | 🔴 Pendiente |
