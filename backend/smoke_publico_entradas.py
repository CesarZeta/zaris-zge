...
Requiere vecino demo con credencial (local DNI 28547123 / prod 30555444, pass [redactado])
y un admin (cesar@municipio.gob.ar / [redactado]) para sembrar un evento si la cartelera
esta vacia.
...
def sembrar_evento_demo(c: httpx.Client) -> bool:
    ...
    h = None
    for email in ADMINS:
        r = c.post(f"{BASE}/api/v1/auth/login", json={"email": email, "password": "[redactado]"})
...
def main() -> int:
    ...
    # 1. Login vecino
    r = c.post(f"{BASE}/api/v1/publico/auth/login", json={"dni": DNI_VECINO, "password": "[redactado]"})
