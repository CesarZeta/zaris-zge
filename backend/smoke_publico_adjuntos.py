...
Requiere: vecino demo DNI 28547123 con credencial activada (pass [redactado]) y,
...
# Vecino demo: local 28547123 / prod 30555444 (ambos pass [redactado]). Override por argv[2].
...
def main() -> int:
    ...
    r = c.post(f"{BASE}/api/v1/auth/login", json={"email": ADMIN_EMAIL, "password": "[redactado]"})
    if r.status_code == 200:
