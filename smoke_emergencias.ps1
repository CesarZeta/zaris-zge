...
#   - usuario operadorcom@municipio.gob.ar (nivel 3, id_subarea=90 Policia, pass [redactado])
#   - vecino demo con credencial activada (pass [redactado]). OJO: el DNI difiere por
...
# ---- login ----
$login = Invoke-Api POST "/api/v1/auth/login" $null @{ email = "ciudadanovl@municipio.gob.ar"; password = "[redactado]" }
...
$loginOp = Invoke-Api POST "/api/v1/auth/login" $null @{ email = "operadorcom@municipio.gob.ar"; password = "[redactado]" }
...
$loginVec = Invoke-Api POST "/api/v1/publico/auth/login" $null @{ dni = "28547123"; password = "[redactado]" }
