...
### Migración 107 + 107b — Historia clínica mínima viable: registro de accesos + clave `id_area_salud` (`backend/migrations/107_historia_clinica_acceso.sql` + `107b_historia_clinica_seed.sql`)

**Aplicadas en local (asyncpg, script multi-statement) Y en prod (conexión directa a Supabase, una transacción por archivo; el MCP no estaba en la sesión) el 2026-09-13.** Verificación independiente en prod: tabla + 2 triggers + RLS + 3 CHECKs + 2 índices; clave `id_area_salud = '57'` → "Secretaría de Salud" activa; primeras 3 filas del log (ok / denegado `fuera_salud` / inexistente) generadas por la verificación post-deploy. F5 del proyecto ATENCIÓN (`PLAN_MODULO_ATENCION.md` §2.4 y §3-F5). Decisión de César (2026-09-01): historia clínica = timeline por ciudadano con DOS fuentes (`turno_atencion` mig 86 + `emergencia_atencion` mig 106), restringida a la gestión Salud + nivel 1 (dato sensible, Ley 25.326 / 26.529). **Sin DDL sobre las fuentes** (los índices por ciudadano ya existían: `idx_turno_atencion_ciudadano`, `ix_emergencia_atencion_ciudadano`).

- **107 (DDL):** tabla **`historia_clinica_acceso`** — una fila por consulta de la historia clínica: `id_usuario` (FK `usuarios` ON DELETE RESTRICT), `id_ciudadano` **sin FK** a propósito (registra también los intentos sobre ids inexistentes — barrido secuencial), `fecha_hora`, `origen` (CHECK `historia|export`, `export` reservado para etapa 2), `contexto` (CHECK `turno|guardia|mesa|consulta|otro`), `resultado` (CHECK `ok|denegado|inexistente`), `motivo` (el evaluado por `permiso_historia_clinica`: `admin|admin_sin_config|salud|nivel|sin_agente|sin_subarea|fuera_salud|sin_config`), `n_registros` (NULL si denegado), `ip`, `user_agent`, `id_municipio`. Sin `activo`/`fecha_modificacion` (es un log, misma excepción al §10 que `emergencia_log`). Índices `ix_hc_acceso_ciudadano (id_ciudadano, fecha_hora DESC)` e `ix_hc_acceso_usuario (id_usuario, fecha_hora DESC)`; RLS deny-all; **append-only** con triggers `historia_clinica_acceso_no_update` / `_no_delete` (fn `historia_clinica_acceso_no_mutate`, patrón mig 83). Verificado en local: UPDATE y DELETE lanzan `historia_clinica_acceso es append-only`.
- **107b (seed, separado por §21):** clave `configuracion_general.id_area_salud` (tipo `integer`, `activo` explícito — en prod son NOT NULL sin default) resuelta **por nombre normalizado** del área ACTIVA "Secretaría de Salud" (**56 local / 57 prod**; las homónimas inactivas 1/7/33 de local quedan afuera por `a.activo = TRUE`), `ON CONFLICT (clave) DO UPDATE`; idempotente. Si el SELECT no encuentra el área, no inserta nada y el guard queda fail-closed para niveles 2-4 (`sin_config`); nivel 1 recibe `admin_sin_config`. Editable en Config → Sistema → "Atención".
- **Código que la usa:** `app/services/historia_clinica.py` (`permiso_historia_clinica` única fuente del permiso, `registrar_acceso`, `SQL_HISTORIA` = `UNION ALL` de las dos fuentes), `turnos.py` (`GET /turnos/atenciones/historia` + `/permiso`, registro dentro de la misma transacción con commit antes de responder — sin fila no hay respuesta; rate limit `hc:{id_usuario}` 60/min), React `web-app/src/modules/turnos/components/HistoriaClinica.tsx`. Smoke `backend/smoke_atenciones_historia.py` 61/61 en local (61 OK / 0 FAIL / 1 SKIP sin `--rate`; con `--rate` el caso 429 también pasa).

### Migración 106 + 106b — Guardia: atención derivada desde Emergencias (`backend/migrations/106_guardia_emergencia_atencion.sql` + `106b_guardia_seed.sql`)

**Aplicadas en local (asyncpg, script multi-statement) y prod (`apply_migration`, las dos) al 2026-09-06.** F4 del proyecto ATENCIÓN (`PLAN_MODULO_ATENCION.md`). Decisión de César del mismo día: la Guardia vive en **Secretaría de Salud, subárea "Emergencias"**; y del 2026-09-01: las derivaciones del COM **no van con turno**.

- **106 (DDL):** (1) `emergencia_log.ck_emergencia_log_tipo_accion` recreado con dos tipos nuevos, `DERIVACION_GUARDIA` y `ATENCION_GUARDIA` (los triggers no-update/no-delete del log no se tocan). (2) Tabla `emergencia_atencion` (§10 + RLS): `id_emergencia_evento` NOT NULL, `id_espacio_ubicacion` NOT NULL (la Guardia), `id_ciudadano` NULL + `paciente_nombre` (el paciente no es necesariamente el denunciante), `motivo_derivacion` NOT NULL, `estado` CHECK `pendiente|atendida|ausente`, `derivado_en`/`id_usuario_deriva`, `id_agente_atiende`/`intervencion`/`recomendaciones`/`atendido_en`, CHECK `atendida ⇒ intervencion + atendido_en`. UNIQUE parcial `uq_emergencia_atencion_pendiente (id_emergencia_evento) WHERE activo AND estado='pendiente'` (una pendiente por evento → 409 al repetir), índices por ubicación+estado y por ciudadano (historia clínica F5).
- **106b (seed, separado por §21):** todo resuelto **por nombre normalizado** (el área de Salud es 56 en local y 57 en prod): subárea `Emergencias` bajo el área activa "Secretaría de Salud" (quedó **115 local / 82 prod**), espacio `Guardia` en esa subárea (`atendido=TRUE`, `prefijo_colero='GUA'`; **12 local / 9 prod**) y clave `configuracion_general.id_espacio_guardia` (tipo `integer`, `activo` explícito) apuntando a ese espacio, con `ON CONFLICT (clave) DO UPDATE`. Idempotente: re-corrido en local sin duplicar.
- **Código que la usa:** `app/services/guardia.py` (clave → espacio, `SELECT_ATENCION`, log sin importar routers), `emergencias.py::derivar_evento_a_guardia` (no toca el FSM), `turnos.py` (`/turnos/guardia/atenciones*`, landing y mesa con `es_guardia`). Smoke 23/23 en local.
...
DDL: `tipo_tramite.id_subarea` (FK `subarea` ON DELETE SET NULL, NULL permitido para tipos en borrador) + índice. **El área NO se guarda: se deriva por JOIN `subarea → area`** (misma regla que `tipo_reclamo`, [[project_tipo_reclamo_area_inconsistencia]]). No toca el FSM ni el scoping de §3 (que lee `agentes.id_subarea`): el destinatario del expediente sigue siendo polimórfico en `tramite`. Backfill de los 15 tipos activos de prod resuelto **por nombre de subárea** (los ids difieren entre entornos) — de paso corrige un desvío real: `aviso-obra` mandaba sus expedientes a Habilitaciones Comerciales en vez de Obras Particulares.

**Trampa §24 cazada aplicando local primero:** la primera versión hardcodeaba `id_area = 1` para reencuadrar las subáreas. En prod el 1 es Gobierno, pero **en local el 1 es "Salud"** → mandó las 7 subáreas del circuito a Salud. Corregido resolviendo el área por nombre (`LIKE '%gobierno%' AND activo`: prod → 1, local → 15 "Secretaria de Gobierno y Legal Tecnica"), y el rename solo matchea el nombre exacto `gobierno` para no tocar el área larga de local.

**Backend:** `id_subarea` en los 3 schemas (`TipoTramiteCreateIn/UpdateIn/AdminOut` + `TipoTramiteAdminListItem`), INSERT del POST, PUT (0 = desasignar, misma semántica que `sla_dias`) y **los DOS SELECT que arman la salida** — detalle `/tipos/{id}/admin` y listado `/tipos` (la trampa recurrente del módulo, [[feedback_columna_nueva_auditar_todos_los_select]]); el listado además ordena área → subárea → tipo. **Frontend:** columna "Gestión" en `ConfigTramites` (subárea + área debajo) y selector agrupado por área en `NuevoTipoModal` y `EditarTipoModal`, que consume `GET /agenda/catalogos/subareas` **por URL directa** para no armar dependencia cruzada entre módulos (mismo criterio que el select de ubicaciones de F2b de Atención).

**`seed_tramites.py` corregido — era el fondo del pedido:** el seed repartía las subáreas entre Gobierno y Servicios Públicos, `id_area_legal` tenía un bug de precedencia (`LIKE '%legal%' OR LIKE '%gobierno%' AND activo` se evalúa `A OR (B AND C)`) y `upsert_subarea` reactivaba una subárea existente **pero nunca le corregía el área** → el ordenamiento dependía del estado previo de la base. Ahora las 7 van al área de gobierno resuelta por nombre (falla ruidoso si no existe), el upsert reencuadra siempre, y un bloque final asigna la gestión de los 15 tipos por mapa código→subárea. Verificado corriendo el seed en local: 9 tipos, 1 sola área, 0 sin gestión.

**En la misma sesión, sin migración:** se configuró el catálogo para que el ciclo de vida de la mig 101 ACTÚE — `espera_iniciador=TRUE` en los 6 estados de espera al vecino (subsanación ×4, pago pendiente, subsanar documentación) y `sla_dias` 2-4 días (`2 + id % 3`) en los 15 tipos activos, local y prod. Los 2 expedientes vivos que caían bajo el timer (`HAB-LPL-2026-0001`, `CAR-LPL-2026-0002`, empresa con mail real, parados 122 y 130 días) tenían el reloj puesto a cero para no dispararles 4 mails y el desistimiento automático. Backups en `_backup_ciclo_vida_2026_09_06` y `_backup_agrupacion_tramites_2026_09_06` (prod).

### Migración 103 — Ubicación de atención (`backend/migrations/103_*.sql`, proyecto Atención F1)

**Aplicada en local y prod al 2026-09-01.** `tipo_prestacion.id_espacio_ubicacion` + `turnos.id_espacio_ubicacion` (copiada al reservar en las 3 vías) + backfill + índice. Detalle del proyecto en `PLAN_MODULO_ATENCION.md`. **Verificado el 2026-09-06:** no hubo que re-correr el backfill tras el deploy — el último turno de prod es del 2026-07-06, anterior a la migración.

### Migración 102 — Foto de perfil del vecino (`backend/migrations/102_ciudadanos_foto_perfil.sql`)

**Aplicada en local (psycopg2) y prod (`apply_migration`) al 2026-08-30.** Esquema propuesto por Roy (2026-07-06) y aprobado por César: `ciudadanos.foto_path` + `foto_actualizada_en` (NULL = sin foto). El binario vive en el bucket privado de adjuntos (`reclamos-adjuntos`, el default de `storage.py`), path fijo `perfiles/{id_ciudadano}.jpg` con `x-upsert` (1 foto por vecino, re-subir pisa, sin huérfanos). Endpoints `POST/DELETE /api/v1/publico/perfil/foto` (multipart, solo JPEG, 1 KB..512 KB, ≥100×100 px — dimensiones leídas del SOF sin Pillow, que no está en requirements) y `foto_url` firmada (TTL 1 h) en `GET /publico/perfil`. Smoke `smoke_publico_perfil_avisos.py` pasos 12b-12h. Contrato §10.

### Migración 101 + 101b — Ciclo de vida del trámite hacia el vecino (`backend/migrations/101_tramites_ciclo_vida_vecino.sql` + `101b_tramites_ciclo_vida_seed.sql`)

**Aplicadas en local (psycopg2) y prod (`apply_migration` + `execute_sql` para el seed) al 2026-08-30.** Decisiones de César del mismo día (4 preguntas cerradas). DDL: `tipo_tramite.sla_dias` (NULL = default global) · `tipo_tramite_estado.espera_iniciador` (flag "espera al vecino") · `tramite.desist_aviso_nivel/desist_aviso_en` (tracking del timer) · `ck_tramite_resultado` + `'desistido'` · `ck_tramite_archivado_motivo` + `'desistimiento'` · ledger + `'aviso_iniciador'` · `ck_ciudadano_aviso_tipo` + `'tramite_estado'`/`'tramite_pendiente'` · `encuesta_envio.id_tramite` (4ª rama, XOR de 4, índice parcial) · índice parcial de trámites vivos. Seed 101b SEPARADO: plantilla CSAT `tipo='tramites'` + 6 claves de `configuracion_general` (`tramite_desistimiento_activo`, `tramite_sla_dias_default`=30, `..._aviso1_dias`=30, `..._aviso2_dias`=60, `tramite_desistimiento_dias`=90, `..._aviso_final_horas`=72; INSERT con `tipo`+`activo` explícitos, `ON CONFLICT (clave)` — UNIQUE verificado en prod). Motor en `services/tramites/ciclo_vida.py`: resultado derivado de la transición final (aprobar/rechazar), aviso PWA + push + encuesta al terminar, timer de desistimiento en el cron diario (avisos 1/2/final + desistido automático como marca paralela), notificación interna al colectivo; `archivar_inactivos` excluye los estados `espera_iniciador`. Builder: SLA en tipo + flag en estado (PATCH in-place `/estados/{id}/espera-iniciador`, editable en versiones publicadas). Smoke `smoke_tramites_ciclo_vida.py` 23/23 (solo local). **Prod arranca con 0 estados marcados `espera_iniciador` y sin `sla_dias` por tipo → el timer no actúa hasta que César configure el catálogo.** Reglas vivas en las skills `modulo-tramites`, `modulo-encuestas` y `modulo-app-vecinos`.

### Migración 100 — `ciudadanos.telefono` VARCHAR(10) → VARCHAR(15) en prod (`backend/migrations/100_ciudadanos_telefono_15.sql`)

**Aplicada en prod (`apply_migration`) al 2026-08-30; en local es no-op (ya estaba en 15).** Drift §24 cazado por el smoke de `PUT /publico/perfil` contra prod: el backoffice (`schemas/buc.py`, `max_length=15`) y local aceptaban hasta 15, prod truncaba en 10 → `StringDataRightTruncation` (500) para cualquier teléfono con espacios/guiones o código de país. Solo ensancha (610 activos, todos ≤ 10 chars). En el mismo commit el `PUT /publico/perfil` normaliza el teléfono con `_validar_telefono_arg` (10 dígitos sin 0 de área, guardado digit-only como la alta pública) y convierte `DataError`/`IntegrityError` del UPDATE en 422.

### Migración 99 — Bandeja de avisos del vecino (`backend/migrations/99_ciudadano_aviso.sql`)

**Aplicada en local (psycopg2) y prod (`apply_migration`) al 2026-08-30.** Tabla nueva `ciudadano_aviso` (FK `ciudadanos` ON DELETE CASCADE, `tipo` con CHECK `reclamo_estado|emergencia_estado|municipio`, `titulo/mensaje/url_destino/recurso_tipo/recurso_id`, `leido/leido_en`, estándar §10 completo con `id_usuario_*` NULL porque el vecino no es `usuarios`) + 2 índices parciales (bandeja por ciudadano/fecha; no leídos) + RLS habilitada sin políticas. Pendiente del traspaso PWA 2026-07-16: la pantalla `/alertas` no tenía backend. Los avisos los persisten los hooks post-commit de `services/push.py` (vía `services/avisos.py`) ANTES del push, así cubren todas las vías de cambio de estado (reclamos.py, ordenes_trabajo.py, emergencias.py) sin tocar los routers. Endpoints `GET/PATCH/POST /api/v1/publico/avisos*` + `avisos.no_leidos` en `/publico/portal/mi-resumen`. En el mismo commit (`d42bed4`): `GET/PUT /publico/perfil` y `GET /publico/reclamos/geo/reverse` (sin migración). Smoke `smoke_publico_perfil_avisos.py` 27/27. Contrato: `docs/contrato_api_publica.md` §4/§10/§11.

### Migración 98 + 98b — Encuestas CSAT para Entradas (`backend/migrations/98_encuestas_entradas.sql` + `98b_encuestas_entradas_seed.sql`)

**Aplicadas en local (asyncpg) y prod (`apply_migration` + `execute_sql` para el seed) al 2026-07-18.** Tercera rama del polimorfismo de `encuesta_envio` (patrón mig 72): `id_evento_reserva` FK a `evento_reservas` con `ON DELETE RESTRICT` (verificado en prod que las FKs de `id_reclamo`/`id_turno` son RESTRICT), `ck_encuesta_envio_origen` XOR de 3, tipo `'entradas'` en `ck_encuesta_plantilla_tipo`, índice parcial. Seed 98b SEPARADO (atomicidad §21): plantilla "Encuesta de asistencia a eventos" (likert5 + texto_libre). El disparo vive en `agenda_v2._patch_reserva_estado` al acreditar (`asistio`), cubre PATCH por id y acreditar-QR. Detalle vivo en la skill `modulo-encuestas`.

### Migración 97 — Alerta de pánico prioritaria en Emergencias (`backend/migrations/97_emergencias_es_panico.sql`)

**Aplicada en local (asyncpg) y prod (`apply_migration`) al 2026-07-18.** `emergencia_evento.es_panico BOOL NOT NULL DEFAULT FALSE` + índice parcial de alertas abiertas + backfill por prefijo normalizado sobre `observaciones_recepcion` (la tabla NO tiene columna `descripcion` — quirk cazado al implementar; 0 filas backfilleadas en prod). El flag lo setea server-side la vía pública del vecino al detectar "ALERTA DE PANICO"; notificación in-app+mail a operadores COM con fallback a admins; tablero con orden es_panico-primero + card destacada + banner + sonido. Detalle vivo en la skill `modulo-emergencias`.

### Migración 96 — Política de renovación de credenciales (`backend/migrations/96_password_renovacion.sql`)

**Aplicada en local (asyncpg) y prod (`apply_migration`) al 2026-07-18.** Idea de César del mismo día. `usuarios.password_actualizada_en TIMESTAMPTZ DEFAULT NOW()` (backfill NOW(): el reloj arranca ese día, nadie es forzado de inmediato) + clave `password_renovacion_dias` en `configuracion_general` (default `365`, `0` = desactivada, editable en Config → Sistema §41). El backend (`auth.py` login, commit `262f152`) marca `debe_cambiar_password=TRUE` al loguear con clave vencida — reusa el gate de cambio forzado de mig 78; el timestamp se reinicia en los 3 puntos de cambio de clave (self-service, recovery por token, reset por admin). **Drift cazado al aplicar en prod:** `configuracion_general.activo` es NOT NULL SIN default en prod (local tiene default TRUE) → el INSERT del seed lleva `activo` explícito (caso §24, tercera vez que muerde).

### Migración 95 — Anti-carrera TOCTOU en turnos y reservas de eventos (`backend/migrations/95_anti_carrera_turnos_reservas.sql`)

**Aplicada en local (asyncpg) y prod (`apply_migration`) al 2026-07-18.** Segunda ronda de la auditoría de seguridad 2026-07 (hallazgos [14][15][37][39]). Tres índices UNIQUE **parciales**: `uq_turnos_slot_agente` y `uq_turnos_slot_espacio` sobre `turnos (recurso, fecha, hora_inicio) WHERE activo AND estado <> 'cancelado'` (un solo turno vigente por slot exacto; cancelar libera el slot), y `uq_evento_reservas_ciudadano_vigente` sobre `evento_reservas (id_evento, id_ciudadano) WHERE activo AND id_estado_reserva <> <cancelada>` (máximo UNA reserva no-cancelada por ciudadano+evento — regla global decidida por César 2026-07-18; el id de `cancelada` se resuelve dinámicamente en un DO block, es 3 en ambos entornos). Pre-verificado 0 duplicados en ambas bases (los pares repetidos históricos incluían una cancelada → fuera del predicado; sin limpieza de datos). En el mismo commit (`b049707`): advisory locks transaccionales (`advisory_lock_tx` por recurso+fecha / ciudadano+día / DNI), `SELECT ... FOR UPDATE` del evento antes del cupo (`lock_evento_row`, 3 vías), `IntegrityError`→rollback+409 en todos los sitios de escritura, CAS de estado en cancelar/cumplir/reprogramar/asistio, guard nuevo `asistio` sobre cancelada→409, y el check de duplicado también en el backoffice de reservas. Smoke de concurrencia in-process 13/13 PASS. Reglas vivas en las skills `modulo-turnos-entradas` y `modulo-agenda`.

### Migración 93 — Geoposicionamiento de espacios de atención (`backend/migrations/93_espacios_lat_lon.sql`)

**Aplicada en local (psql) y prod (`apply_migration`) al 2026-07-02.** Agrega `espacios_agenda.latitud`/`longitud NUMERIC(10,7) NULL` (mismo tipo que reclamos/BUC/emergencias). Motivación: el Dashboard "Resumen de actividad municipal" muestra capas geoposicionadas — espacios con pin propio, y turnos/entradas ubicados vía su espacio. En la misma sesión: endpoint agregado `GET /api/v1/dashboard/resumen` (router `dashboard.py`, conteos de 6 tarjetas + 4 capas geo en una pasada, `::float` en lat/lon porque NUMERIC sale Decimal→string en dicts crudos), `EspacioFormModal` con bloque §23 (`DireccionGeoField`, compone `GeocodingSearch`+`MapaPicker` de Reclamos), y el campo dinámico `direccion` de Trámites pasó a persistir `{texto, lat, lon}` en `datos_jsonb` (retro-compat con string plano; validador en `services/tramites/creacion.py`). Reglas vivas en CLAUDE.md §4 (fila Dashboard) y §23.

### Migración 89 — Tipo `bloqueo` en ocupaciones (`backend/migrations/89_ocupaciones_tipo_bloqueo.sql`)

**Aplicada en local (psql) y prod (`apply_migration`) al 2026-06-12.** Recrea los CHECKs de `ocupaciones`: `ck_ocup_tipo` acepta `bloqueo` (4º tipo) y `ck_ocupacion_consistencia` gana la rama `bloqueo` con las 3 FKs NULL. Permite el cierre manual de un recurso sin entidad asociada (espacio en mantenimiento, agente/equipo afectado a otra tarea) — antes ningún tipo lo permitía Y `existe_recurso` ni siquiera validaba espacios (404 en el POST manual, hallazgo de la revisión integral de Agenda). El backend exige `motivo` (schema `OcupacionCreate`); UI en `OcupacionModal` (tipo bloqueo + select de espacios); bloque gris en grilla; `descripcion_corta_sql` rama `'Bloqueo: <motivo>'`. En el mismo commit (`cda8585`) fue el guard `require_operador` (nivel ≤ 3) en las 14 mutaciones de `agenda_v2.py`. Verificado E2E en prod: bloqueo del Salón de Usos Múltiples excluyó los slots del autoservicio de turnos y al eliminarlo volvieron; consultor nivel 4 real recibe 403 al mutar y 200 al leer. Reglas vivas en CLAUDE.md §27.

### Migración 88 — Foto de perfil del usuario (`backend/migrations/88_usuarios_foto_url.sql`)

**Aplicada en local y prod al 2026-06-12.** Agrega `usuarios.foto_url VARCHAR(500)` (NULL/vacía = avatar con iniciales). La sube el propio usuario desde el dropdown del topbar del shell ("Cambiar foto…", PNG/JPG ≤2MB): `POST /auth/me/foto-upload-url` firma la URL (bucket público `config-assets`, paths `usuarios/{id}/avatar-{uuid}.{ext}`, mismo flujo que el logo del municipio §26) → PUT del binario directo a Storage → `PUT /auth/me/foto` persiste. `foto_url` viaja en `/auth/login` y `/auth/me` (SELECT de `get_current_user`). En la misma sesión se entregó el resto del rediseño del menú de usuario: `cargo_nombre` derivado del agente vinculado (sin migración), modo oscuro del DS (`[data-theme="dark"]`, §13) y link a Guías. Reglas vivas en CLAUDE.md §13/§14.

### Migración 87 — Bbox de geocoding configurable (`backend/migrations/87_geo_bbox_config.sql`)

**Aplicada en local y prod al 2026-06-12.** Tres claves nuevas en `configuracion_general` (`geo_bbox_centro_lat` `-34.5305`, `geo_bbox_centro_lon` `-58.4779`, `geo_bbox_delta_grados` `0.27`, tipo `string` — son decimales) que hacen configurable el sesgo geográfico del buscador de direcciones (§23), antes hardcodeado en constantes `GEO_BBOX_*` de `geo.py`. El backend las lee en `geo.py::_bbox_municipio()` con cache TTL 5 min y sesión propia (`AsyncSessionLocal`, porque `geocodificar_direccion` corre también desde routers públicos sin DB inyectada); las constantes quedan como fallback ante claves ausentes/inválidas. Editables en Config → Sistema (sección "Búsqueda de direcciones"). Idempotente (`ON CONFLICT DO NOTHING`), DO block que detecta la columna `tipo` (drift prod/local, patrón 75b). Verificado en local con test en vivo (config + Nominatim, resultados dentro del bbox).

### Migración 86 — Historia de atención en Turnos (`backend/migrations/86_turno_atencion.sql`)

**Aplicada en local y prod al 2026-06-11.** Agrega `tipo_prestacion.registra_atencion` (BOOL NOT NULL DEFAULT FALSE — marca a nivel prestación, NO del espacio, que al cumplir cada turno se registra una atención) y crea **`turno_atencion`** (1:1 con el turno por UNIQUE `id_turno`, FKs a `turnos`/`ciudadanos`, `intervencion` NOT NULL + `recomendaciones`, estándar §10, índice parcial por `id_ciudadano`, RLS deny-all — dato de salud sensible Ley 25.326). Registro genérico de atención (sirve para atención médica o regular), append-only de hecho. Post-mig se marcaron por nombre (`ILIKE '%odontolog%'`) las dos prestaciones demo de Odontología en local y prod (UPDATE separado del DDL). Reglas vivas en CLAUDE.md §33 ("Historia de atención").

### Migración 91 — Override de disponibilidad de equipos sin agentes (`backend/migrations/91_equipos_disponibilidad_override.sql`)

**Aplicada en local y prod al 2026-06-15.** Data-only: clave `configuracion_general.equipos_sin_agentes_usan_horario_propio` (boolean, default `'false'`, editable en Config → Sistema §41). Acompaña el cambio de modelo de esa sesión: la disponibilidad de una cuadrilla es la **unión de los horarios de sus agentes** (no su horario propio) — ver §27. Con el override en `true`, un equipo SIN agentes usa su horario propio cargado en `disponibilidad_recurso(tipo_recurso='equipo')`. El INSERT detecta la columna `tipo` (NOT NULL en prod, ausente en local — drift §24/§41) con el patrón DO-block de `75b`. Idempotente (`ON CONFLICT DO NOTHING`). Contexto del bug que motivó el cambio (dos endpoints calculaban la disponibilidad de equipo distinto y divergían) en memoria [[feedback_misma_magnitud_dos_endpoints_diverge]].

### Migración 90 — Recovery interno + DNI de agente (`backend/migrations/90_recovery_interno_y_dni_agente.sql`)

**Aplicada en local y prod al 2026-06-13** (commit `360cba7`). Recovery de credenciales para usuarios internos + DNI en agentes. (Bitácora detallada pendiente — registrada acá para mantener la numeración; el detalle vive en el commit.)

### Migración 85 — Numerador anual de Emergencias (`backend/migrations/85_emergencias_numerador_anual.sql`)

**Aplicada en local y prod al 2026-06-10** (cierre del pendiente "reset anual" de la mig 83). Reemplaza la secuencia global `emergencia_evento_numero_seq` por la tabla **`emergencia_numerador (anio PK, ultimo_numero)`** con UPSERT atómico dentro de `fn_generar_numero_emergencia` (patrón `tipo_tramite_numerador` §35): año nuevo ⇒ fila nueva que arranca en 1 — **reset anual implícito, sin cron**. Backfill idempotente (`GREATEST` sobre el máximo ya emitido por año) y `DROP SEQUENCE` de la global. RLS deny-all. Verificado: local continuó la numeración sin saltos (EM-2026-000026+); prod, que no tenía eventos, arrancó en `EM-2026-000001`. En la misma sesión se cerró la **Fase 5** del plan (router `publico_emergencias.py`, sin migración — reusa migs 82-83), el smoke creció a 64 asserts (scoping nivel 3 + Fase 5), se generó `docs/manual_emergencias.html` y se sembraron datos demo permanentes en prod (eventos EM-1..11 en todos los estados + operador COM + vecino demo DNI 30555444).

### Migración 84 — Permiso del módulo Emergencias (`backend/migrations/84_modulo_emergencias_permisos.sql`)

**Aplicada en local y prod al 2026-06-10** (Fase 4 del plan). Fila `emergencias` en el catálogo `modulos` (§30) con `min_nivel_acceso=3` — habilita el ítem del sidebar `data-modulo="emergencias"` (primero del nav, pedido del usuario) y espeja el guard nivel≤3 del backend. Frontend React `web-app/src/modules/emergencias/` (Tablero con polling 30s + Recepción de llamado + Detalle con FSM/historial) registrado en el shell React y en `index.html`. Tokens `--prio-p1/p2/p3` agregados al DS y a `tokens.css`.

### Migración 83 — Eventos de Emergencias + log append-only (`backend/migrations/83_emergencias_eventos.sql`)

**Aplicada en local y prod al 2026-06-10** (Fase 3 del plan `PLAN_MODULO_EMERGENCIAS.md`). Crea `emergencia_contacto_eventual` (denunciante cargado al vuelo, índices por dni/teléfono/nombre), `emergencia_evento` (transaccional principal: FKs a catálogos mig 82 + `subarea`/`ciudadanos`/`usuarios`, CHECK de denunciante anónimo-XOR-BUC-XOR-eventual, CHECK de veracidad, timestamps de KPI recepción/despacho/arribo/cierre) y `emergencia_log` (**append-only**: sin `activo`/`fecha_modificacion`, triggers `emergencia_log_no_update`/`_no_delete` que rechazan toda mutación — verificados con UPDATE/DELETE reales en local). Numeración `EM-YYYY-NNNNNN` vía secuencia + trigger BEFORE INSERT `trg_numero_emergencia` (patrón `trg_nro_reclamo`), verificado con `pg_trigger` en prod. RLS deny-all en las 3 tablas. Endpoints de Fase 3 en `routes/emergencias.py` (contactos eventuales + búsqueda unificada de denunciantes + eventos con FSM y promoción a BUC); smoke `smoke_emergencias.ps1` 43/43 OK en local.

### Migración 82 — Catálogos del módulo Emergencias (`backend/migrations/82_emergencias_catalogos.sql`)

**Aplicada en local y prod al 2026-06-10** (Fase 2 del plan `PLAN_MODULO_EMERGENCIAS.md`). Crea las 6 tablas catálogo del COM: `emergencia_organismo_derivacion`, `emergencia_canal_ingreso`, `emergencia_prioridad`, `emergencia_estado`, `emergencia_tipo` (FK a `subarea` + UNIQUE `(id_subarea, codigo)`), `emergencia_subtipo` (FK a tipo + UNIQUE `(id_tipo, codigo)`). PKs `id_<tabla>`, estándar §10 completo, RLS deny-all en las 6. Solo DDL — el seed va aparte: `backend/seed_catalogos_emergencias.py` lee `Tablas Iniciales/emergencia_*.csv` y genera SQL idempotente (ON CONFLICT, FKs por código/nombre normalizado); modo `--emit-sql` para prod vía MCP. Seedeado en ambos entornos con conteos idénticos: 34 tipos Policía Municipal (taxonomía RESO-2022-166 MinSeg PBA) / 16 tipos Defensa Civil / 147 subtipos / 12 organismos / 7 estados / 2 canales / 3 prioridades. Router de lectura `routes/emergencias.py` (7 GET, guard JWT a nivel router) registrado en `main.py`.
...
- **B2 Frontend** ✅ 2026-05-14 (commit `505c185`): 4 tabs (Vistas/Eventos/Conflictos/Config), sub-toggle Día/Semana/Mes, pills de tipo de recurso, CRUD de espacios y disponibilidad. Componentes: `VistasView`, `WeeklyView`, `ConfigView`, `RecursoTogglePills`, `EspaciosConfig`, `DisponibilidadConfig`, etc. Hallazgo fixeado: drift `id_municipio NULL` entre `/recursos/conteos` y grilla.
- **Pendientes post-B2** todos cerrados 2026-05-14 (commit `257b9a8`): items `data-modulo` duplicados, eventos sin asignar en grilla, badge falta-vincular-agentes, drag en vista Semana, KeyboardSensor, título residual.
...
Módulo completo en `web-app/src/modules/tramites/`. Pusheado en commit `dfcc396`.
...
### Editor admin de tipos custom (✅ ENTREGADO 2026-05-18, commit `20a868b`)
...
**Mejoras de UX del editor de campos (hallazgos QA 2026-05-27, commits `05eba5d`+`4cf1bb9`):** el `CampoModal` autocompleta el nombre interno desde la etiqueta visible (auto-slug, ver §23), valida en vivo con indicador ✓/✕, edita las opciones de `seleccion`/`seleccion_multiple` con filas {Etiqueta, Valor} en vez del textarea `valor|Etiqueta`, y `_modalShell` ya no cierra al arrastrar texto fuera del modal (click-outside exige mousedown+mouseup sobre el overlay). La lista de campos tiene botones ↑↓ (`useReordenarCampo` — ver issues 2026-05-28 abajo: reasigna `orden` 1..N por posición, NO swap). BUG-01 "Failed to fetch" del reporte era blip transitorio de Railway (§9), no código.

**Vista previa del formulario en vivo (BUG-06, 2026-05-27, commit `1743c3c`):** la tab Campos tiene un toggle **Editar / Vista previa**. En "Vista previa", `admin/components/PreviewFormulario.tsx` renderiza el formulario de inicio reusando el **mismo** `FormularioDinamico` de la pantalla de alta real (`CrearTramite`) — interactivo (botón "Probar validación") pero no guarda. Se actualiza solo al editar campos (react-query invalida la versión). **Quirk cazado:** `CampoDinamico` ahora normaliza `opciones_jsonb` tolerando el shape legacy `{opciones:[...]}` de los tipos seedeados viejos (antes `.map` reventaba); esto **también protege el alta real**. Ver [[feedback_normalizar_jsonb_de_seeds_viejos]]. El manual `docs/manual_admin_tramites.html` quedó actualizado a esta UI (commit `f20e389`).

**Issues del editor de tipos — repaso de Roy (2026-05-28, mig 68, commit `abae1e1`):** 6 hallazgos del editor de "Tipos de trámite", todos resueltos y verificados en navegador (local + prod):
...
**Issues del ALTA de trámite — repaso de Roy (2026-05-29, commits `761ae07`+`30cf70a`, solo frontend):** el alta (`CrearTramite.tsx`) estaba rota por **dos mismatches de contrato frontend↔backend**, ambos verificados en vivo contra prod (Aviso de Obra):
...
| `frontend/admin_tablas.html` | 0 (desde `042dd03`) | 5 (solo `z-header*` oculto en iframe) | ✅ tokens DS directos. Clases internas (`.btn-primary`, `.field`, `.modal`) se conservan a propósito — ver §15. |
| `frontend/login.html`, `welcome.html` | 0 | 0 | ✅ |

> **Nota 2026-05-12:** los HTMLs `ciudadano.html`, `empresa.html`, `reclamos.html` (y sus JS) fueron eliminados al migrar a React (commits `5cb6910`, `f0adbab`, `435296a`-`3bd9fde`). Las equivalencias de tokens/clases listadas más abajo siguen siendo útiles si en algún momento se reintroduce un módulo vanilla nuevo.
>
...
`admin_tablas.html` fue migrado a tokens DS directos en commit `042dd03` (2026-05-13): 0 `var(--z-*)` remanentes. Las clases internas (`.btn-primary`, `.field`, `.modal`) se conservan a propósito (renombrarlas a `*-zaris` colisionaría con el DS sin ganancia funcional). No queda deuda de estilos legacy en el repo.
