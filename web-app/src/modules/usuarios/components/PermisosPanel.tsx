...
// Hooks/API de permisos viven en el módulo config (cross-module import §29).
import {
  useModulosCatalogo,
  usePermisosUsuario,
...
// Matriz de permisos por módulo de UN usuario, embebida en el detalle del
// maestro (merge UX 2026-07-16: antes era una solapa aparte con su propio
// selector de usuario — UsuariosPermisosView, eliminada).
// Estado por modulo: 'default' (sigue por nivel), 'allow' (override true), 'deny' (override false)
type EstadoModulo = 'default' | 'allow' | 'deny'

export function PermisosPanel({ idUsuario }: { idUsuario: number }) {
  ...
  if (permisosQ.isLoading || modulosQ.isLoading) {
    return <div style={emptyStyle}>Cargando permisos…</div>
  }
  if (permisosQ.isError || modulosQ.isError) {
    return <div style={{ ...emptyStyle, color: 'var(--color-error)' }}>No se pudieron cargar los permisos.</div>
  }
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 16 }}>
        <button onClick={guardar} disabled={mut.isPending} style={btnPrimary}>
          {mut.isPending ? 'Guardando…' : 'Guardar permisos'}
        </button>
      </div>
    </div>
  )
...
const emptyStyle: React.CSSProperties = {
  padding: 32, textAlign: 'center', color: 'var(--fg-3)', fontSize: '0.88rem',
}
