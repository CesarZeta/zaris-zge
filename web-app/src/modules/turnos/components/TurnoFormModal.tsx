...
import { useCrearTurno, useReprogramarTurno, usePrestaciones } from '../hooks/useTurnos'
...
export function TurnoFormModal({ open, onClose, turno }: Props) {
  ...
  const [idPrestacion, setIdPrestacion] = useState<number | ''>('')
  const [fecha, setFecha] = useState(HOY())
  const [horaInicio, setHoraInicio] = useState('09:00')
  const [observaciones, setObservaciones] = useState('')

  // Reset / hidratacion al abrir (patron CLAUDE.md §29).
  useEffect(() => {
    if (!open) return
    if (turno) {
      setCid(null)
      setIdPrestacion(turno.id_tipo_prestacion)
  ...
  const prestSel = prestaciones.data?.find((p) => p.id_tipo_prestacion === idPrestacion)

  async function onSubmit() {
    if (!esEdicion && !cid) {
      push({ kind: 'error', title: 'Elegí un ciudadano' }); return
    }
    if (idPrestacion === '') {
      push({ kind: 'error', title: 'Elegí una prestación' }); return
    }
    try {
      if (esEdicion && turno) {
        await reprogramar.mutateAsync({
          ...
          body: {
            id_tipo_prestacion: idPrestacion,
            fecha,
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {/* Prestacion */}
        <div>
          <label style={lbl}>Prestación</label>
          <select
            value={idPrestacion}
            onChange={(e) => setIdPrestacion(e.target.value === '' ? '' : Number(e.target.value))}
            style={inp}
          >
            <option value="">Elegí una prestación…</option>
            {(prestaciones.data ?? []).map((p) => (
              <option key={p.id_tipo_prestacion} value={p.id_tipo_prestacion}>
                {p.nombre} ({p.duracion_min} min)
              </option>
            ))}
          </select>
          {prestSel && (
            <div style={hint}>
              Atiende: <strong>{prestSel.recurso_nombre ?? '—'}</strong>
              {' '}({prestSel.tipo_recurso === 'espacio' ? 'lugar de atención' : 'agente'})
              {' '}· duración {prestSel.duracion_min} min. La hora de fin se calcula automáticamente.
            </div>
          )}
          {(prestaciones.data ?? []).length === 0 && !prestaciones.isLoading && (
            <div style={hint}>No hay prestaciones cargadas. Crealas en la pestaña Prestaciones.</div>
          )}
        </div>
