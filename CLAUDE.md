...
El directorio `web-app/` contiene un **shell React contenedor** (`AppShell` + sidebar + topbar propios) que **solo se usa en desarrollo local** (`localhost:5173`) para iterar módulos React sin tener que levantar el shell vanilla. **En producción, ese AppShell se auto-oculta** (regla §14) y el módulo React vive embebido en el iframe del shell vanilla. **No hay dos shells en producción** — hay uno solo (vanilla) que carga módulos de distintos stacks. **Desde 2026-08-31 el AppShell standalone CLONA visualmente el shell vanilla de prod** (grid 52/1fr/30, topbar con brand+municipio+contexto de usuario, sidebar `.nav-flat`, statusbar — pedido de César para que la vista previa dev refleje la interfase real): si cambiás el look del shell vanilla (`index.html`/`frontend/css/menu.css`), replicá el cambio en `web-app/src/shell/*` para que dev no vuelva a divergir.
...
- **Mapas (módulos React):** **Leaflet 1.9 vanilla** (`leaflet` + `@types/leaflet`, sin `react-leaflet`). React 19 + react-leaflet v5 tuvo bugs de compat al cierre 2026-05-12; el patrón usado es montar el mapa en `useEffect` con `useRef<L.Map>` y mantener `onChange` estable vía `useRef` callback. Referencia: `web-app/src/modules/reclamos/components/MapaPicker.tsx`. Workaround obligatorio para iconos del marker (Vite rompe los paths default): import explícito de los PNG (`marker-icon-2x.png`, `marker-icon.png`, `marker-shadow.png`) y `L.Marker.prototype.options.icon = L.icon({...})`.
  - **Elegir tile basemap según uso:**
    ...
    - **Dashboards / mapas con markers** (`DashboardMap`, `EmergenciasMap`): **Esri World Light Gray Base** `https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Light_Gray_Base/MapServer/tile/{z}/{y}/{x}` (sin subdominios, **`maxNativeZoom: 16` + `maxZoom: 20`** — Esri no tiene tiles nativos más allá de z16 en la zona, Leaflet escala). Gris claro minimal, gratis, sin API key. Atribución: `© Esri © OpenStreetMap`. **NO usar CartoDB Positron/Dark Matter (`basemaps.cartocdn.com`): desde 2026-08-30 sirven los tiles con marca de agua "API KEY REQUIRED"** (cazado en prod por César; si algún día se quiere volver a CARTO hay que crear una API key en carto.com).
...
- **Un query param que el endpoint NO declara, FastAPI lo IGNORA en silencio** (no da 422). El frontend manda el filtro, la request sale 200 y el listado viene **sin filtrar** — parece que "el filtro no anda" pero no hay ningún error en ningún lado, y el typecheck del front no lo ve porque el tipo TS lo definís vos. **Antes de agregar un filtro nuevo en el frontend, leé la firma del handler** y confirmá que ese param exista ([[feedback_verificar_firmas_api_antes_de_consumir]]). Cazado 2026-09-06: el panel del colero filtraba por `fecha` y `GET /turnos` sólo acepta `fecha_desde`/`fecha_hasta` → habría listado los turnos de todos los días.
- **Token UUID por path en endpoint PÚBLICO: validarlo con `uuid.UUID(...)` ANTES de la query.** Un `CAST(:tok AS uuid)` con basura tira `invalid input syntax for type uuid` = **500** en una ruta que cualquiera puede llamar sin auth (logs llenos de excepciones a discreción de un tercero). Token inválido e inexistente son el mismo caso para quien mira: **404**. Cazado en prod 2026-09-06 en `/turnos/publico/pantalla/{token}` — el token válido daba 200 y el inexistente 404, pero `no-es-un-uuid` daba 500. Aplica a `token_turno`, `token_reserva`, `token_publico`, `token_pantalla` y cualquier token nuevo.
- **Orden de routers FastAPI con `{param}` greedy en main.py:** si registrás un router nuevo bajo un prefix cuyo tronco lo comparte otro router que usa `/{param}` greedy (como `admin_tablas` con `/{tabla}` y `/{tabla}/{id}`), el router específico **debe registrarse ANTES** del genérico. Sino FastAPI matchea por orden de registro y atrapa la ruta nueva como si fuera `{tabla}='lo-que-sea'`. Síntoma: 422 con `int_parsing` en `{id}` (porque `{id}` no es int). Caso real sesión 2026-05-12: `admin_permisos` bajo `/api/v1/admin/permisos/*` atrapado por `admin_tablas` con `{tabla}='permisos'`, `{id}='modulos'`. Fix: invertir el orden en `main.py`.
  - **Dentro de un mismo router, el greedy `/{param}` atrapa exactamente UN segmento** (verificado con `starlette.routing.compile_path`: `^/prefix/(?P<param>[^/]+)$`, 2026-09-13). Una ruta nueva de un solo segmento (`/turnos/historia`) declarada después de `GET /{id_turno}` da 422 `int_parsing`; una de dos o más (`/turnos/atenciones/historia`) no colisiona. Regla: **ruta fija nueva → dos segmentos o más, y declarada ANTES del greedy igual** (convención del archivo); probar `GET /prefix/<id-existente>` después para confirmar que el greedy sigue vivo. Caso F5.
- **Query param que se persiste en una columna `INTEGER` → acotarlo con `le=2_147_483_647`** (y `offset` con un `le` razonable). `Query(..., ge=1)` acepta cualquier entero de Python; asyncpg lo bindea como int4 y con `3000000000` el INSERT tira `DataError: value out of int32 range` = **500** — y si ese INSERT era el registro de auditoría, el intento **no queda registrado**. Con la cota es 422 antes de tocar nada. Cazado por el smoke de F5 (2026-09-13) en `historia_clinica_acceso.id_ciudadano`.
...
**El edge de Railway corta TODA respuesta a los ~300 s (502 "upstream error") aunque el proceso siga corriendo.** Un endpoint que pueda tardar más — generadores demo, backfills, exports pesados, cualquier cosa con cientos de queries encadenadas (cada round-trip Railway↔Supabase cuesta ~0,5-3 s; la misma corrida tardó 1 s en local y 14,5 min en prod) — tiene que ser **asíncrono**: validar, encolar en `BackgroundTasks` con sesión SQL propia (`AsyncSessionLocal`, memoria `feedback_background_tasks_sesion_nueva`), responder **202** con un id y exponer un `GET` de estado para que el workflow haga polling. Y al revés: **un `failure` de un workflow que ESCRIBE datos NO prueba que no escribió nada** — verificar en la DB (timestamps de las filas dentro de la ventana del run) ANTES de re-dispatchar. Patrón de referencia: `POST /demo/poblar` + `GET /demo/poblar/{id_corrida}` + `.github/workflows/demo-datos.yml` (2026-09-22; el run del 2026-09-21 quedó en `failure` con los 4 pasos commiteados).
...
- **Mapas Leaflet**: en dark usar **Esri World Dark Gray Base** (`Canvas/World_Dark_Gray_Base`, misma URL base y `maxNativeZoom` que el light de §4 — CARTO `dark_all` quedó descartado por la API key). `DashboardMap.tsx` lo elige leyendo `document.documentElement.dataset.theme` al montar.
...
**Estado general:** migraciones 20-107b aplicadas en local Y prod (Supabase) sin divergencia conocida (95 = anti-carrera turnos/reservas; 96 = renovación de credenciales `password_actualizada_en`; 97 = alerta de pánico `es_panico`; 98+98b = encuestas de Entradas — todas 2026-07-18; 99 = bandeja de avisos del vecino `ciudadano_aviso`; 100 = `ciudadanos.telefono` a VARCHAR(15) en prod, drift cazado; 101+101b = ciclo de vida del trámite hacia el vecino (SLA, `espera_iniciador`, desistimiento, encuestas de trámites); 102 = foto de perfil del vecino `ciudadanos.foto_path` — las cuatro 2026-08-30; 103 = ubicación de atención en prestaciones/turnos, 2026-09-01; 104 = `tipo_tramite.id_subarea` (gestión responsable del tipo) + área 1 renombrada a «Secretaría de Gobierno», 2026-09-06). La numeración 51 está duplicada (`51_notificaciones.sql` + `51_tramites_tipo_dato_direccion.sql`, ambas aplicadas) — 104 = `tipo_tramite.id_subarea`; 105 = ciclo de llamado + colero de Turnos, 2026-09-06) — 106 + 106b = Guardia (`emergencia_atencion`, tipos `DERIVACION_GUARDIA`/`ATENCION_GUARDIA` en el CHECK del log; seed de subárea "Emergencias" bajo Secretaría de Salud + espacio "Guardia" + clave `id_espacio_guardia`), 2026-09-06 — 107 + 107b = historia clínica F5 (log de accesos `historia_clinica_acceso` append-only + clave `id_area_salud` por nombre del área activa "Secretaría de Salud", 56 local / 57 prod), 2026-09-13 — **cualquier mig nueva debe usar 108+**. El número se asigna al APLICAR, no al planificar (la 104 "reservada" para el colero la tomó Trámites).

**Reglas vivas de migración:**
- **Entidad de negocio que el código necesita ubicar (el área "Salud", el espacio "Guardia", el bbox del municipio) → clave en `configuracion_general` sembrada POR NOMBRE en la mig, leída POR ID en runtime.** Nunca `ILIKE '%nombre%'` en el código: en local `'%salud%'` matchea 4 áreas (3 inactivas, una con subárea ACTIVA donde cuelga un supervisor) y los ids difieren entre entornos (Salud 56 local / 57 prod). El seed exige `activo = TRUE`, el `ON CONFLICT` NO pisa `valor` (respeta lo que el admin cambió en Config → Sistema) y el lector es fail-closed si la clave falta/apunta a algo inactivo (`CASE WHEN valor ~ '^[0-9]{1,9}$'`, sin 500 por basura). Patrón: `id_espacio_guardia` (106b), `id_area_salud` (107b), `geo_bbox_*` (87). Cazado en F5 (2026-09-13).
- **El número de migración se asigna al APLICARLA, no al planificarla.** Un `PLAN_*.md` puede reservar números para fases futuras, pero si otra tarea aplica una migración antes, esa se queda con el número y **hay que renumerar el plan** (no dejar dos migs con el mismo número ni saltear). Caso 2026-09-06: la 104 estaba reservada para el colero de Atención y la tomó `tipo_tramite.id_subarea` → el colero pasó a 105 y la guardia a 106.
...
- **Implementación de referencia:** helper compartido `web-app/src/ui/busqueda.tsx` (`useBusquedaDiferida` = borrador/aplicado/version + `AvisoBuscar`; promovido desde Turnos el 2026-09-20); los hooks de listado aceptan `opts: { enabled?, version? }` con default `enabled: true` (`version` en la queryKey para que Buscar con los mismos filtros vuelva a la red; con `enabled: false` react-query tampoco pollea, así que un `refetchInterval` recién corre tras la primera búsqueda). El toolbar es un `<form onSubmit>` (Enter = Buscar) y todo botón que no busca lleva `type="button"`. Refrescar/Exportar/Imprimir quedan deshabilitados hasta la primera búsqueda; contadores/chips que consultan al backend (StatsBar de Reclamos, StatsChips de OT) se muestran recién después. Ejemplos por variante: filtros server (`turnos/pages/Overview.tsx`, `reclamos/views/ListView.tsx`, `ot/views/SupervisorView.tsx`), sin filtros server = botón "Ver mis X" (`ot/views/AgenteView.tsx`, `entradas/pages/Overview.tsx`), grilla = "Ver agenda" (`agenda/views/VistasView.tsx`, flag `agendaBuscada` en el store en memoria), filtros en la URL (`tramites/pages/BandejaTramites.tsx`: la URL es lo aplicado, el borrador es local, Buscar escribe todo de una vez + `buscar=1`).
- **Alcance — ESTÁNDAR DE TODA LA SUITE (decisión de César 2026-09-20):** aplicado en Turnos, Reclamos (listado), OT (mesas Supervisor/Agente/Auditoría), Trámites (bandeja y mi bandeja), Agenda (grillas Día/Semana/Mes, eventos, conflictos), Entradas (eventos), Encuestas (envíos) y los listados completos de Ciudadanos/Empresas. **Intención explícita cuenta como búsqueda:** llegar con filtros en la URL (deep link a la bandeja de Trámites) o vía el botón "Listado" del buscador BUC (`navigate('/x/listado', { state: { buscar: true } })`) arranca ya buscado. **Excluidos a propósito (no gatear):** tableros y dashboards (inicio con mapa, Datos/BI, Encuestas → Resumen), pantallas operativas sin filtros de búsqueda (Emergencias: tablero del COM y Recepción; Mesa del día; Guardia; colero), pantallas que SON el selector (Ubicaciones de Turnos), buscadores search-first con preview liviana §15 (Ciudadanos/Empresas → Buscar, Usuarios) y catálogos/configuración sin filtro (Plantillas de encuesta, catálogo de módulos, Espacios/Disponibilidad/Feriados de Agenda, builder de Trámites). Los catálogos que pueblan los selects de filtro (áreas, tipos, espacios, pills con conteos) sí cargan al entrar: son las opciones del filtro. **Toda pantalla de listado NUEVA nace con este patrón.**
...
- **Mantenerlo corto: tamaño objetivo ~12 KB; si al cerrar sesión supera ~15 KB, se poda en ese mismo cierre.** "Hecho reciente" se poda (máx ~10 líneas); lo viejo se borra (ya está en el historial); "Última actualización" es la foto de la ÚLTIMA sesión, no acumulativa; un ítem tachado que ya no aporta contexto se borra, no se deja tachado. (El 2026-09-20 llegó a 63 KB por acumular; la poda del 2026-09-22 lo bajó a 13 KB.) Verificar contra git/prod antes de declarar algo hecho (`feedback_verificar_siempre_antes_de_opinar`).
