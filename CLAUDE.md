...
- **PASO 2 — completar ficha:** `POST /publico/auth/completar-ficha` (**JWT scope publico**, `get_current_ciudadano`). El vecino ya verificado/logueado carga su ficha real (CUIL módulo-11 válido, sexo, fecha nac, nacionalidad, domicilio OSM, teléfono). Reemplaza los placeholders y marca `ficha_completa=TRUE`. El `id_ciudadano` SIEMPRE sale del token (no del body). Rechaza CUIL de otro ciudadano (409). **Frontend del paso 2 = PWA `zaris-vecinos`** (otro repo): pantalla post-login que se muestra cuando `me.ficha_completa===false`.
- **Camino B (alta por agente, §38 `/publico/auth/registrar`):** alineado en Fase 4 con `ficha_completa=FALSE` en el INSERT. El agente da de alta SIN password; el vecino recibe mail, clickea ACTIVAR, **elige su propia clave** (no hay clave temporal que cambiar) y al entrar el portal lo lleva a completar la ficha (mismo paso 2). Decisión 2026-06-09: NO se usa clave-temporal+cambio-forzado para vecinos (el flujo de activación ya fuerza al vecino a elegir su clave).
