...
### Alta pública de vecinos (autoregistro) — ENTREGADO 2026-06-01
Página pública `frontend/alta-vecino.html?m=<slug>` (vanilla, DS ZARIS) donde el **vecino se da de alta a sí mismo** (sin agente, sin JWT). Router `routes/publico_alta.py` (`/api/v1/publico/alta/*`, sin JWT, rate-limited por IP). **Tenancy mono-tenant validado**: el slug `?m=<codigo_corto>` se valida contra el ÚNICO municipio del deploy (`municipios.codigo_corto`, ej. `lpl`) → 404 si no coincide. URL preparada para multi-deploy sin reescribir a multi-tenant.
- **Crea cuenta + ficha JUNTAS en una tx**: `ciudadanos` (`estado_validacion='auto_registrado'`, `email_chk=FALSE`) + `ciudadano_credencial` (password + `token_activacion`, `activado=FALSE`). El **mail de verificación activa la cuenta** (`GET /verificar` → `email_chk=TRUE`, `estado_validacion='verificado'`, `activado=TRUE`). El vecino NO es `usuarios` (scope `publico`, [[project_usuario_vs_ciudadano_modelo]]).
- **Empresa exige ciudadano previo** (BUC §2): `empresas` + `ciudadano_empresa` + `empresa_credencial` (mig 76, solo verificación de email, SIN password). Verificar → `empresas.email_chk=TRUE` + `empresa_credencial.verificado=TRUE`.
- **Paridad de campos con el alta oficial** (módulos React ciudadanos/empresas): CUIL/CUIT reales validados módulo 11 (reusa `_validar_modulo11` de `schemas/buc.py`), sexo, fecha nac, nacionalidad, **domicilio normalizado por OSM**, observaciones. NO usa los placeholders del `/registrar` de agente.
- **Geocoding OSM público**: `GET /publico/alta/geo/buscar?m=&q=` reusa `geocodificar_direccion()` extraída de `geo.py` (el `/geo/buscar` autenticado quedó como wrapper delgado). El front replica el `AddressSearch.tsx` en vanilla (debounce 500ms + dropdown + skip-post-pick §29). El `/geo/buscar` del backoffice SIGUE exigiendo JWT — solo el público es abierto.
- Endpoints: GET `/identidad` · `/actividades` · `/nacionalidades` · `/geo/buscar` (todos validan slug) + POST `/ciudadano` · `/empresa` + GET `/verificar` (devuelve **página HTML** de confirmación, la abre el navegador). Link del mail = `FRONTEND_BASE_URL/api/v1/publico/alta/verificar` (no la PWA — el dominio del shell). Templates `enviar_mail_verificacion_alta_ciudadano/_empresa` en `services/email.py`. Verificado E2E navegando (local + prod) + smoke API.
...
- **Autoservicio del vecino logueado (Etapa 2, 2026-06-02, commit `53edc28`, en prod):** dos routers nuevos con guard `get_current_ciudadano` (scope `publico`), SIN migración (reusan columnas existentes):
  - **`publico_reclamos.py` (`/api/v1/publico/reclamos`):** POST crea reclamo a nombre propio (`id_ciudadano` del token; `id_usuario_alta=NULL` — no es `usuarios`, columna nullable; `canal_origen='app_movil'`; estado inicial 'Sin asignar'; deriva `id_area` de `subarea.id_area` vía el tipo §27; valida tipo activo) + GET lista SOLO los suyos (`WHERE id_ciudadano=<token>`) + GET `/{id}` detalle con **guard duro 404 si no es suyo** (mismo cuerpo que "no existe", no filtra terceros; historial sin identidad del agente) + GET `/catalogo/tipos` (declarado ANTES de `/{id_reclamo}` por el param greedy §5).
  ...
  - Ambos registrados junto a los `publico_*` en `main.py` (prefijos no colisionan con `reclamos_router` ni `publico_alta`). Smoke local 9/9 + deploy prod verificado (`/openapi.json` + 401 sin token). **Verificación E2E con token en prod PENDIENTE** (hoy no hay ningún ciudadano con credencial activada en prod). **v1 NO incluye:** cambiar estado, cancelar reclamo propio, "mis-turnos"/"mis-entradas" logueados (turnos/entradas siguen por token anónimo §33), adjuntos públicos. Las pantallas viven en la PWA `zaris-vecinos` (otro repo). **Pendiente UI backoffice (no codeado):** previsualizar en Config → Identidad las URLs públicas customizadas del municipio (cambian por slug).
