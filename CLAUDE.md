...
- **Permisos del vecino**: acciones de autoservicio acotadas a SUS datos (paridad con operador "en su nombre", NO backoffice). Los endpoints de autoservicio (crear reclamo/ver turnos desde la cuenta) **NO existen aún** — `get_current_ciudadano` solo se usa en `/me`. Es la próxima etapa. Ver [[project_usuario_vs_ciudadano_modelo]].
