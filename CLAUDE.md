...
- **Turnos:** GET `` (filtros estado/agente/espacio/ciudadano/prestación/fecha), GET `/{id}`, POST `` (recurso+duración salen de la prestación; calcula `hora_fin`), PUT `/{id}` (reprograma — solo `reservado`), PATCH `/{id}/cumplir`, PATCH `/{id}/cancelar`. Mutar turnos: nivel ≤ 3.

**Frontend:** `web-app/src/modules/turnos/` — `TurnosLayout` con 2 tabs (**Turnos** = lista/alta/reprogramar/cumplir/cancelar; **Prestaciones** = ABM del catálogo, **visible solo nivel ≤ 2** vía `hasPermission(2)`). `TurnoFormModal` ahora elige prestación (sin selector de recurso, que es read-only de la prestación). `PrestacionFormModal` tiene form condicional: clase `atencion` → toggle agente/espacio; `reserva_espacio` → solo espacio. Reusa `Modal`, `ConfirmModal`, `RecursoPicker`, `useEspacios` de Agenda.
