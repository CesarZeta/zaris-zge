...
En prod: lo mismo via `execute_sql` Supabase MCP. Password de todos los devs: `[redactado]` (set por `seed_auth.py`).
...
### Turnos — turnos de atención sobre agente O lugar de atención

Un turno reserva un bloque de la disponibilidad de un **agente** o de un **lugar de atención atendido** (`espacios_agenda`) para que un ciudadano realice un trámite (tipo de servicio). Estados: `reservado` → `cumplido` | `cancelado`.

> **Modelo polimórfico (mig 70, 2026-05-28):** `turnos.id_agente` e `turnos.id_espacio` son ambos nullable + CHECK `ck_turnos_recurso` (exactamente uno). Un turno apunta a un agente O a un espacio atendido (capacidad = su disponibilidad efectiva = unión de los agentes vinculados §27). Backoffice y autoservicio ofrecen ambos. La ocupación espejo usa `tipo_recurso='agente'` o `'espacio'` según corresponda. `TurnoOut` expone `recurso_tipo`/`recurso_nombre` derivados. Endpoint público nuevo `/api/v1/turnos/publico/recursos` (agentes con disponibilidad + espacios atendidos); `/slots` acepta `tipo_recurso`+`id_recurso` (con compat retro `id_agente`).

> **Switch global `turnos_respeta_disponibilidad`** (`configuracion_general`, mig 69, default `true`): con `true` el alta de turno (backoffice + autoservicio) exige que caiga en la **disponibilidad efectiva** del recurso (horario − feriados − novedades de agentes §27); con `false`, modo libre. Helper `services/agenda.py::turnos_respeta_disponibilidad(db)`. El anti-solapamiento contra `ocupaciones` siempre aplica.

**DB (migración 45 `45_turnos.sql`, aplicada local + prod 2026-05-14; ampliada por mig 70):**
...
- `turnos`: tabla transaccional (estándar §10). FKs a `ciudadanos`, `agentes` (nullable), `espacios_agenda` (nullable, mig 70), `tipo_servicio_turno`. `estado` CHECK `reservado|cumplido|cancelado`. `id_ocupacion` → fila espejo en `ocupaciones`.
- **Seed demo:** `backend/seed_turnos_demo.py` (idempotente) siembra disponibilidad de agentes, 1 lugar atendido + 1 desatendido y turnos demo (incluye al ciudadano modelo Juan Pérez id 1).
