...
- **Direcciones normalizadas con OSM (2026-06-10):** dirección del EVENTO con `GeocodingSearch` de Reclamos (POIs válidos; el pick normaliza + captura lat/lon, editarla a mano limpia las coords) y domicilio del CONTACTO EVENTUAL con `AddressSearch` (`solo_direcciones=true`) pero **editable como fallback** — una emergencia no se frena por el geocodificador (a diferencia del readonly de Ciudadanos).
