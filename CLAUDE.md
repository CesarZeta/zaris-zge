...
### Marca `resultado` del trámite + política de retención de documentos (mig 74, Fase 1)
`tramite.resultado` (`pendiente|aprobado|rechazado`, CHECK `ck_tramite_resultado`) es una marca **paralela al estado FSM**, NO un estado: el estado es del flujo del circuito, el resultado dice cómo concluyó el trámite (se consulta junto al estado, "archivado, aprobado/rechazado"). Endpoint `POST /tramites/{ref}/resultado` (nivel ≤ 2, `ResultadoIn`) + movimiento `'resultado'` en el ledger (tipo agregado al CHECK `tramite_movimiento_tipo_check` en mig 74). Expuesto en `TramiteDetalleOut.resultado` — **setearlo en los DOS builders del detalle** (`_tramite_detalle_out` + el GET handler `detalle_tramite`, [[feedback_columna_nueva_auditar_todos_los_select]]). Frontend: `ResultadoChip` (chip + dropdown, solo nivel ≤2).
- **Política de retención (documentada en `docs/manual_tramites.html` §10, marcada como "prevista"):** el **registro** de cada documento (metadatos + hash) NUNCA se borra; solo se depura el **binario físico** por antigüedad. **Aprobado → 10 años; rechazado/descarte → 1 año.** Trámite sin movimiento 180 días → cron lo archiva + marca `resultado='rechazado'` por inactividad. Excepción configurable por tipo de trámite ("nunca depurar adjuntos", ej. Habilitaciones). **Solo la Fase 1 (la marca) está implementada al 2026-06-01; Fases 2-5 (flag por tipo, auto-archivado 180d, depuración, cron) pendientes.**
