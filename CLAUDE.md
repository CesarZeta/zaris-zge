...
- **Filtros Prestación / Atiende(agente|lugar) / Ciudadano** en Turnos (`Overview`) y Agenda, vía helper compartido `lib/turnoFiltros.tsx` — opciones derivadas de los turnos cargados (no catálogos completos, §23), client-side, combinables, con botón Limpiar. **No hay botón "Buscar"**: los filtros aplican en vivo y el Exportar PDF exporta lo visible.
