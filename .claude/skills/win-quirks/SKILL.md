...
- **Q15 — `& "C:\Program Files\...\psql.exe"` lo bloquea el PowerShell tool** como falso positivo "Remove-Item path protegido" (parsea mal ruta-con-espacios + `&`). Para DB LOCAL en smokes usar script Python (`@'...import asyncpg/psycopg2...'@ | python`), no `& $psql`. Prod = `execute_sql`/`apply_migration` MCP. [[feedback_ps_quirks_startprocess_psql]] (Quirk 3). *(Nota 2026-06-11: en esta instalación `& $psql` SÍ corre desde el PowerShell tool — si lo bloquea, caer al fallback Python.)*
  ...
  - **Q15c — el here-string del script Python va SINGLE-quoted `@'...'@`, NUNCA `@"..."@`**: el double-quoted interpola variables PS, y los placeholders posicionales de asyncpg (el signo dólar seguido de un número) quedan VACÍOS → el SQL llega roto (`UPDATE ... SET x =  WHERE ...`, "error de sintaxis cerca de WHERE" con encoding raro). Cazado 2026-06-12 asignando un cargo por asyncpg.
...
**Q19 — Mensaje de commit multilínea: `git commit -F <archivo>`, NUNCA here-string con comillas dobles adentro.** PS 5.1 rompe el paso de args nativos cuando el here-string `@'...'@` contiene `"` embebidas: git recibe pedazos del mensaje como pathspecs ("error: pathspec 'turno' did not match…") y el commit NO se hace (cazado 2026-07-18, commit de mig 95). Receta: escribir el mensaje a un archivo del scratchpad (Write tool, UTF-8) y `git commit -F "<ruta>"`. Para mensajes de una línea, `-m` sin comillas dobles ni em-dashes va bien.

**Q20 — Script Python dentro de un heredoc del Bash tool: NO anidar comillas triples.** Un `python - <<'EOF' … EOF` cuyo script contiene `'''` (o un literal `'''…'''` que a su vez menciona comillas triples) termina en "unexpected EOF" / `SyntaxError` (cazado dos veces el 2026-08-30 — la segunda, documentando este mismo quirk). Receta segura: el TEXTO largo va a un archivo del scratchpad con `cat > "$TEMP/x.md" <<'EOF'` (sin pasar por Python) y el script Python (corto, con `"""`) lo lee y lo inserta. Para archivos con acentos abrir con `encoding='utf-8'`; para los `.tsx` del repo (LF) escribir con `newline` LF.

**Q21 — `TaskStop` de un `pnpm dev` / `uvicorn` lanzado con `run_in_background` NO libera el puerto.** Mata el bash envoltorio; el hijo (`node`, `python`) sigue escuchando (cazado 2026-08-30: 5173 respondía 200 tras el stop). Matar **por puerto**: `Get-NetTCPConnection -LocalPort 5173 -State Listen | % OwningProcess | % { Stop-Process -Id $_ -Force }` y verificar con `Get-NetTCPConnection` de nuevo (o un `curl` que falle). Familia de [[feedback_uvicorn_restart_tras_registrar_routers]] y de la regla global "verificación independiente".

**Q22 — `https://cesarzeta.github.io/zaris-zge/...` responde `301` al dominio propio `https://zge.zaris.com.ar/...`.** Un poller que espera `200` sobre la URL `github.io` sin `-L` nunca corta (cazado 2026-08-30). Para verificar que GH Pages sirve el bundle nuevo: `curl -sL` o directamente la URL en `zge.zaris.com.ar`, y grepear un string que SOLO exista en el commit nuevo.

**Q23 — `uvicorn --reload` lanzado desde el Bash tool en background puede NO recargar.** Cazado 2026-09-06: `python -m uvicorn … --reload 2>&1 | tail -n +1` arrancó, pero tras editar `app/schemas/agenda_v2.py` el OpenAPI local siguió sin el campo nuevo (con `watchfiles` instalado). No confiar en el reload: ante un cambio de schema/router **verificar `curl /openapi.json`** y, si no aplicó, matar el ÁRBOL (bash → python supervisor → worker; `taskkill /PID` en Git Bash se convierte en un path — usar `Get-CimInstance Win32_Process | ? { $_.ProcessId -eq $pid -or $_.ParentProcessId -eq $pid } | % { Stop-Process -Id $_.ProcessId -Force }`) y relanzar SIN pipe, encadenando kill + sleep + uvicorn en el MISMO comando background (así el orden queda garantizado). Familia de Q21 y [[feedback_uvicorn_restart_tras_registrar_routers]].

**Q24 — Un script Python largo dentro de un heredoc del Bash tool falla al parsear aunque no tenga comillas triples.** Variante de Q20 (cazada 2026-09-06 con un patch de ~150 líneas: "unexpected EOF while looking for matching `''`"): el heredoc `<<'EOF'` de Git Bash se corta con ciertos contenidos largos con comillas simples/dobles mezcladas. Receta: **escribir el script a un archivo del scratchpad con la herramienta Write y correrlo con `python "<ruta>"`**; que el script verifique cada reemplazo (fragmento único antes de escribir) y relea el archivo después ([[feedback_write_por_script_puede_no_persistir]]). Los post-checks por **presencia** de un fragmento, no por conteo (los conteos mal estimados dan falsos FAIL con el archivo ya escrito).

**Q25 — El Bash tool bloquea `sleep N` encadenado con otros comandos ("To wait for a condition, use Monitor…").** Para esperar un deploy/CI: `run_in_background: true` con un `until <check>; do sleep 30; done` acotado por iteraciones (una sola notificación al terminar; salir con código distinto de 0 en timeout para no confundir silencio con éxito). Cazado 2026-09-20 esperando que GH Pages sirviera el bundle nuevo.
