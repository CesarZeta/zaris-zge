...
- **2026-06-11 — ETAPA A desplegada en prod.** Backend en Railway (commits `ddc7a02`+`4fd2b2c`, deploy verificado vía `/openapi.json` en ~40s) + smoke contra prod **12/12 OK** con cleanup (adjunto borrado vía DELETE admin, reclamo de prueba soft-deleted por SQL). PWA en Vercel **READY** (commit `9940fb9`). **Dato corregido:** el vecino demo de PROD es DNI `30555444` ("Vecino Demo"; credencial en `credenciales-testing/`, fuera del repo) — el `28547123` existe SOLO en local; el smoke ahora elige el DNI por entorno (override por argv[2]). Queda solo la prueba en Android real del usuario.
...
- **2026-06-11 — ETAPA C implementada, verificada E2E y desplegada en prod.** ZGE `6aba636` + PWA `0e5ecc7`:
...
- **2026-06-12 — REPLANTEO: alta en UN PASO con ficha completa (ZGE `e344c40`, PWA `0e97a3e`) — EN PROD.** Decisión del usuario (AskUserQuestion): (1) el alta nunca dentro de la app; única vía = página pública con TODA la ficha y validaciones canónicas; (2) ambas vías para vecinos pre-existentes. Reemplaza el modelo de 2 pasos de Fase 4 (2026-06-09, duró 3 días: los placeholders falsos en la BUC — fecha 1900, CUIL inventado — fueron rechazados de plano).
  ...
  - **Deuda de manuales SALDADA (2026-06-12, commit ZGE `9888b39`):** `manual_alta_vecino.html` reescrito al flujo de un paso (6 secciones, suma la vía "ya estoy registrado") y `manual_alta_ciudadanos.html` patcheado (sección 5 + botón "Crear cuenta App Vecinos" + footer sin fecha). La card de Guías ya era precisa.
- **2026-06-12 — ETAPA D implementada, verificada E2E en LOCAL y PROD.** ZGE `88e4a4b` + PWA `612fbb1`:
...
- **2026-06-12 — ETAPA E implementada y desplegada (backend + PWA).** ZGE `a2af197` + PWA `db3c4da`:
